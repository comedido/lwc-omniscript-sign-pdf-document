...
export default class SignPdfDocument extends OmniscriptBaseMixin(LightningElement) {
    ...
    renderedCallback() {
        ...
        this.communityName = urlString.substring(urlString.indexOf(".com") + 5, urlString.indexOf("s/") - 1);

        // Resolve Static Resource path dynamically (community vs platform)
        let resourcePath = this.communityName.length < 25 ? '/' + this.communityName + '/resource' : '/resource';
        ...
        let pdf_lib = resourcePath + "/pdf_lib";
        Promise.all([
        ...
        let signature_pad = resourcePath + "/signature_pad";
        Promise.all([
    ...
    async signPdfDocument() {
        ...
        let datasourcedef = JSON.stringify({
            "type": "dataraptor",
            "value": {
                "bundleName": "DocumentInsertPdfDocument",
                "inputMap": payload
            }
          });
